...
:: 开始打包后端
call mvn -f "%PROJECT_ROOT%elysiananime-server\pom.xml" clean install -Dmaven.test.skip=true
if %errorlevel% neq 0 (
    echo Error occurred while installing elysiananime-server.
    exit /b %errorlevel%
)
echo elysiananime-server install finish.
